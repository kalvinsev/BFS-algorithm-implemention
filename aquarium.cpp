...
void Aquarium::interact() {
	...
	for (int i = 0; i < m_nActors - 1; i++) {
		Point pos = m_actors[i]->currentPosition();
		for (int j = i + 1; j < m_nActors; j++) { // j never 0
			if (pos == m_actors[j]->currentPosition()) {
...
void Aquarium::draw() const {
	...
	for (int i = 0; i < m_nActors; i++) {
		...
		char& c = m_renderedAquarium[pos.x()][pos.y()];
		switch (c) {
	...
	if (!(m_player->stuck() || m_player->foundExit())) {
		...
		if(target.x()>=0 && target.y() >= 0)
			m_renderedAquarium[target.x()][target.y()] = 'T';
	}
...
bool Aquarium::isOpen(Point p) const {
...
int Aquarium::addSharks() {
	...
	do {
		...
		p = m_openCells.get(index);
	
	} while ((p == m_player->currentPosition())); // Don't start where the player starts
